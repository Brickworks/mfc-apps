...
fn sys_report() -> Vec<ReportMessage> {
    ...
    msgs.push(ReportMessage{
        level: LogLevel::Info,
        content: format!("total memory: {} kB", system.get_total_memory()),
    });
    msgs.push(ReportMessage{
        level: LogLevel::Info,
        content: format!("used memory : {} kB", system.get_used_memory()),
    });
    msgs.push(ReportMessage{
        level: LogLevel::Info,
        content: format!("total swap  : {} kB", system.get_total_swap()),
    });
    msgs.push(ReportMessage{
        level: LogLevel::Info,
        content: format!("used swap   : {} kB", system.get_used_swap()),
    });
    return msgs
}
