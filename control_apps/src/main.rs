...
fn test_control_mngr() {
    ...
    pwms = ctrl_manager.update(target_altitude - 500.0, 1.0);
    pwms = ctrl_manager.update(target_altitude - 450.0, 0.8);
    pwms = ctrl_manager.update(target_altitude + 100.0, 0.5);
    pwms = ctrl_manager.update(target_altitude, 0.2);
    pwms = ctrl_manager.update(target_altitude, 0.0);
    pwms = ctrl_manager.update(14999.9, 1.0);
}
